...
class %BOOTCODE%_ClosureCompiler {
    ...
    public function min($js)
    {
        ...
        if (preg_match('/^Error\(\d\d?\):/', $response)) {
            if (is_callable($this->_fallbackFunc)) {
                ...
                throw new %BOOTCODE%_Minify_JS_ClosureCompiler_Exception($response);
            }
        }
        if ($response === '') {
            $errors = $this->_getResponse($this->_buildPostBody($js, true));
            throw new %BOOTCODE%_Minify_JS_ClosureCompiler_Exception($errors);
        }
        return $response;
    }
    ...
    protected function _getResponse($postBody)
    {
        ...
        if (false === $contents) {
            throw new %BOOTCODE%_Minify_JS_ClosureCompiler_Exception(
